...
\textbf{Caso I.} Supongamos que $f$ es una función continua en el intervalo $[a,\infty)$. La integral impropia $\int_{a}^{\infty }f\left( x\right)\,dx$ se evalúa de la siguiente manera (figura \ref{inimpainf}):
\begin{gather*}
...
\begin{figure}[H]
  \centering
  \caption{Integral impropia a $\infty$}\label{inimpainf}
  \includegraphics[width=7cm]{Gráficas/IntegImprIntervDerech}
  \propia
\end{figure}

\textbf{Caso II.} Sea $f$ una función continua en $(-\infty ,b]$. La integral impropia $\int_{-\infty }^{b}f\left( x\right)\,dx$ se evalúa de la siguiente manera (figura \ref{intimpdesmininf}):
\[
...
\begin{figure}[H]
  \centering
  \caption{Integral impropia desde $-\infty$}\label{intimpdesmininf}
  \includegraphics[width=6.5cm]{Gráficas/IntegImprIntervIzqu}
  \propia
\end{figure}
...
\begin{figure}[H]
  ...
  \includegraphics[width=6cm]{Gráficas/IntegImprReales}
  \propia
\end{figure}
...
\begin{ejemplo}
  Encuentre el valor de la integral $\int_{2}^{\infty }\frac{1}{\left( x+1\right) \left( x+2\right) }dx$.
\end{ejemplo}
...
\begin{align*}
  ...
  x+2\right\vert \\
  &=\ln \left\vert \frac{x+1}{x+2}\right\vert
\end{align*}
...
\begin{align*}
  ...
  e^{2w}\right)\\
  &=\frac{1}{2}e^{6}-0=\frac{1}{2}e^{6}=201.71
\end{align*}
...
\begin{align*}
  ...
  \infty }\int_{2}^{z}\frac{2}{\left( x+1\right) ^{2}}dx=\left.\lim_{z\rightarrow \infty }\left( \frac{-2}{x+1}\right)\right\vert_{2}^{z} \\
  &=\lim_{z\rightarrow \infty }\left( \frac{-2}{z+1}-\frac{-2}{3}\right) \\
  &=0+\frac{2}{3}=\frac{2}{3}
\end{align*}
...
\begin{align*}
  \int \frac{dx}{x\left( x^{2}+1\right) }&=\ln \left\vert \sen \theta\right\vert=\ln \left\vert \frac{x}{\sqrt{x^{2}+1}} \right\vert\\
  &=\ln \left\vert \sqrt{
...
Con ello,
\begin{align*}
  \int_{1}^{\infty }\frac{8}{x^{3}+x}dx&=8\lim_{z\rightarrow \infty }\int_{1}^{z}\frac{1}{x\left( x^{2}+1\right) }dx \\
  &=8\left.\lim_{z\rightarrow \infty }\frac{1}{2}\ln \left\vert \frac{x^{2}}{x^{2}+1}\right\vert\right\vert_{1}^{z}\\
  &=4\lim_{z\rightarrow \infty }\left( \ln \left\vert \frac{x^{2}}{x^{2}+1}%
  \right\vert -\ln \frac{1}{2}\right) \\ \displaybreak
  &=4\left( \ln \left\vert \lim_{z\rightarrow \infty }\frac{z^{2}}{z^{2}+1}%
  \right\vert -\ln \frac{1}{2}\right) \\
  &=4\left( \ln 1-\ln \frac{1}{2}\right) \\
  &=4\left( -\ln \frac{1}{2}\right) =4\ln 2\approx 2.7726%
\end{align*}
...
\end{ejemplo}
\emph{Solución.} Si reescribimos la integral y hacemos la sustitución $y=x^2$, \mbox{tenemos}
\[
  \int_{2}^{\infty }\frac{x}{x^{4}-1}dx=\int_{2}^{\infty }\frac{x\,dx}{\left(
...
\begin{align*}
  \int_{3}^{\infty}\frac{x}{x^{4}+3x^{2}-4}dx&=\int_{3}^{\infty }\frac{x\,dx}{%
...
\begin{ejemplo}
  Diga si la integral $\int_{1}^{\infty}\frac{dx}{e^{x+2}+e^{4-x}}$ converge o diverge. Si converge, diga a qué valor lo hace.
\end{ejemplo}
...
\begin{ejemplo}
  Diga si la integral $\int_{2}^{\infty }\frac{x^{2}}{\left( x^{2}+4\right) ^{2}}dx$ converge o diverge. Si converge, diga a qué valor lo hace.
\end{ejemplo}
...
\begin{gather*}
  \begin{aligned}
    ...
    }}\frac{2}{\sqrt{x^{2}+4}}\right) =\frac{1}{4}\left( \tan^{-1} \left( \frac{x}{2}\right) -\frac{2x}{x^{2}+4}\right)
  \end{aligned}
\end{gather*}
...
\begin{ejemplo}
  Diga si la integral $\int_{1}^{\infty }\frac{4}{x\left( x+1\right) ^{2}}dx$ converge o diverge. Si converge, diga a qué valor lo hace.
\end{ejemplo}

\emph{Solución.} Al descomponer en fracciones parciales el integrando, se tiene
\begin{align*}
  \frac{4}{x\left( x+1\right)^{2}}&=\frac{A}{x}+\frac{B}{x\left( x+1\right) }+\frac{C}{x}\\
...
\begin{align*}
  ...
  &=4\ln x-4\ln \left( x+1\right) +\frac{4}{x+1}\\
  &=4\ln \left\vert \frac{x}{x+1}\right\vert +\frac{4}{x+1}
\end{align*}
...
\textbf{Caso I.} Sea $f$ una función continua en el intervalo $(a,b]$ y supongamos que $x=a$ es una asíntota vertical de $f$. Entonces, la integral $\int_{a}^{b}f\left( x\right)\,dx$ es impropia y, para calcularla, se procede con el siguiente límite:
\[
...
\begin{figure}[H]
  ...
  \includegraphics[width=5.5cm]{Gráficas/IntegImprDisconLimferior}
  \propia
\end{figure}

\textbf{Caso II.} Supongamos que $f$ es una función continua en $[a,b)$ y que $x=b$ es una asíntota vertical de $f$. La integral impropia $\int_{a}^{b}f\left( x\right)\,dx$ se define como
\[
...
\begin{figure}[H]
  ...
  \includegraphics[width=5.5cm]{Gráficas/IntegImprDisconLimSuperior}
  \propia
\end{figure}

\textbf{Caso III.} Sea $f$ una función continua en $[a,b]-\left\{ c\right\} $, para $c\in\left( a,b\right)$, y $x=c$ es una asíntota vertical de $f$. La integral impropia $\int_{a}^{b}f\left( x\right)\,dx$ se define como
\begin{gather*}
  \begin{aligned}
...
\begin{figure}[H]
  ...
  \includegraphics[width=5.5cm]{Gráficas/IntegImprDisconIntervalo}
  \propia
\end{figure}
...
\begin{ejemplo}
  Calcule la siguiente integral: $\int_{0}^{1}\frac{dx}{\sqrt{1-x^{2}}}$.
\end{ejemplo}

\emph{Solución.} En este caso, tenemos una discontinuidad en el límite superior para el integrando; entonces,
\begin{gather*}
  \begin{aligned}
    \int_{0}^{1}\frac{dx}{\sqrt{1-x^{2}}}&={\lim_{c\to 1^{-}} \int_{0}^{c}\frac{dx}{\sqrt{1-x^{2}}}} =\lim_{c\to 1^{-}} \left( \int \frac{dx}{\sqrt{1-x^{2}}}\right)\Bigg|_{0}^{c}\\
    &=\lim_{c\to 1^{-}}\arcsin (x)\big| _{0}^{c}
    =\lim_{c\to 1^{-}}\left( \arcsin (c)-\arcsin (1)\right)\\
    &=\lim_{c\to 1^{-}}\left( \arcsin (c)-0\right)
    =\lim_{c\to 1^{-}}\arcsin (c)=\frac{\pi }{2}
  \end{aligned}
\end{gather*}

\begin{ejemplo}
  Calcule la siguiente integral: $\int_{-1}^{1}\frac{dx}{x^{\frac{2}{3}}}$.
\end{ejemplo}

\emph{Solución.} \textls[-10]{En este caso, tenemos una discontinuidad en un punto del intervalo. El integrando es discontinuo en $x=0$, que está en el intervalo $[-1,1]$:}
\begin{align*}
    \int_{-1}^{1}\frac{dx}{x^{\frac{2}{3}}}&= \int_{-1}^{0}\frac{dx}{x^{\frac{2}{3}}}+\int_{0}^{1}\frac{dx}{x^{\frac{2}{3}}}
    =\lim_{c\rightarrow 0^{-}}\int_{-1}^{c}\frac{dx}{x^{\frac{2}{3}}}+\lim_{z\rightarrow 0^{+}}\int_{z}^{1}\frac{dx}{x^{\frac{2}{3}}} \\ 
    &=\lim_{c\rightarrow 0^{-}}\left( \int \frac{dx}{x^{\frac{2}{3}}}\right)\Bigg|_{-1}^{c}+\lim_{z\rightarrow 0^{+}}\left(\int \frac{dx}{x^{\frac{2}{3}}}\right)\Bigg|_{z}^{1}\\ \displaybreak
    &=\lim_{c\rightarrow 0^{-}}\left( \int x^{-\frac{2}{3}}dx\right)\Bigg|_{-1}^{c}+\lim_{z\rightarrow 0^{+}}\left(\int x^{-\frac{2}{3}}dx\right)\Big|_{z}^{1} \\
    &=\lim_{c\rightarrow 0^{-}}\left( 3x^{\frac{1}{3}}\right)\Big|_{-1}^{c}+\lim_{z\rightarrow 0^{+}}\left( 3x^{\frac{1}{3}}\right)\Big|_{z}^{1} \\ 
    &=\lim_{c\rightarrow 0^{-}}\left( 3c^{\frac{1}{3}}-3(-1)^{\frac{1}{3}}\right) +\lim_{z\rightarrow 0^{+}}\left( 3(1)^{\frac{1}{3}}-3(z)^{\frac{1}{3}}\right) \\
    &=\lim_{c\rightarrow 0^{-}}\left( 3c^{\frac{1}{3}}+3\right)+\lim_{z\rightarrow 0^{+}}\left( 3-3(z)^{\frac{1}{3}}\right)
\end{align*}

Para calcular el límite anterior, recordemos que debemos encontrar el límite de cada término; entonces, $\lim_{c\rightarrow 0^{-}}\big( 3c^{\frac{1}{3}}\big) =3(0)^{\frac{1}{3}}=0$, $\lim_{c\rightarrow 0^{-}}\left( 3\right) =3$, $\lim_{z\rightarrow 0^{+}}(3) =3$ y $ \lim_{z\rightarrow 0^{+}}\big( 3(z)^{\frac{1}{3}}\big) =0$. Con ello se tiene que
\[
...
\begin{align*}
  \int_{a}^{b}f(x)\,dx\approx M_{n}&=\sum\limits_{i=1}^{n}f(x_{i}^{*})\Delta x\\
  &=\Delta x[f(x_{1}^{* })+f(x_{2}^{*})+\ldots +f(x_{n}^{*})]
\end{align*}
...
\begin{align*}
  ...
  &=\frac{1}{8}+\frac{27}{8}+\frac{125}{8}+\frac{343}{8}+\frac{729}{8}=\frac{1225}{8}
\end{align*}
...
\begin{figure}[H]
  ...
  \includegraphics[width=8cm]{Gráficas/rtrapecio}
  \propia
\end{figure}
...
\end{gather*}
Los puntos de la partición son $x_{0}=a$, $x_{1}=a+\Delta x$, $x_{2}=a+2\Delta x$, $x_{3}=a+3\Delta x,\ldots,x_{n-1}=a+(n-1)\Delta x$\; y\; $x_{n}=a+n\Delta x=b$\; y también
\[
  \Delta x=\frac{b-a}{n}
\]
...
\begin{align*}
  P&=\left\{1,1+\frac{1}{4},1+2\left(\frac{1}{4}\right),1+3\left(\frac{1}{4}\right),1+4\left(\frac{1}{4}\right)\right\}\\
  &=\left\{1,\frac{4+1}{4},1+\frac{1}{2},1+\frac{3}{4},1+1\right\}\\
  &=\left\{1,\frac{4+1}{4},\frac{2+1}{2},\frac{4+3}{4},2\right\}
  =\left\{1,\frac{5}{4},\frac{3}{2},\frac{7}{4},2\right\}
\end{align*}
...
\begin{gather*}
  \begin{aligned}
    ...
    &=\frac{1}{8}\left[ e+2e^{4/5}+2e^{\frac{2}{3}}+2e^{4/7}+e^{1/2}\right]\\
    &=0.125(2.7183+4.4511+3.8955+3.5416+1.6487)\\
    &=0.125(16.255)=2.0319
  \end{aligned}
\end{gather*}

\begin{ejemplo}
  Aproxime la $\int_{0}^{3}\frac{1}{1+x^{4}}\ dx$ con $n=6$ usando la regla del trapecio.
\end{ejemplo}

De la integral dada tenemos que $f(x)=\frac{1}{1+x^{4}}$; $\Delta x=\frac{3-0}{6}=\frac{3}{6}=\frac{1}{2}$, y la partición es
\begin{gather*}
...
\begin{gather*}
  \begin{aligned}
    V&=\int_{0}^{50}30h(x)\,dx \approx T_{10}\\
    &=30\left( \frac{1}{2}\right) \left[
...
\begin{figure}[H]
  ...
  \includegraphics[width=8cm]{Gráficas/simpson.pdf}
  \propia
\end{figure}

Para el método de Simpson, en lugar de usar rectángulos o trapecios como en otros métodos, aproximamos el área bajo la curva de la función $f$ por medio de regiones dadas por partes de parábolas. En cada par de subintervalos consecutivos, aproximamos la curva $y=f(x)\geq 0$ mediante una parábola que pasa por los puntos consecutivos $(x_{i-1},y_{i-1}),(x_{i},y_{i}),(x_{i+1},y_{i+1})$ de la curva ({figura~\ref{punforpar}}).

\textls[-5]{La ecuación de una parábola es $y=Ax^{2}+Bx+C$; por tanto, el área debajo de la parábola desde $x_{0}=-h$, $x_{1}=0$ a $x=h$, con $h=\Delta x=\frac{b-a}{n}$ es}
\begin{align}
    A_{1} &= \frac{Ax^{3}}{3}+\frac{Bx^{2}}{2}+Cx\Big|_{-h}^{h} \nonumber \\
    &=\frac{Ah^{3}}{3}+\frac{Bh^{2}}{2}+Ch-\left[ \frac{A(-h)^{3}}{3}
    +\frac{B(-h)^{2}}{2}+C(-h)\right] \nonumber \\ \displaybreak
    &=\frac{Ah^{3}}{3}+\frac{Bh^{2}}{2}+Ch-\left[ \frac{-Ah^{3}}{3}
    +\frac{Bh^{2}}{2}-Ch\right] \nonumber \\
    &=\frac{Ah^{3}}{3}+\frac{Bh^{2}}{2}+Ch+\frac{Ah^{3}}{3}
    -\frac{Bh^{2}}{2}+Ch \nonumber \\
    &=\frac{2Ah^{3}}{3}+2Ch \nonumber \\
    &=\frac{h}{3}\left[ 2Ah^{2}+6C\right] \label{eq:e51}
\end{align}

\begin{figure}[H]
  \centering
  \caption{Puntos que forman la parábola}\label{punforpar}
  \includegraphics[width=7cm]{Gráficas/02_rgsimpson.pdf}
  \propia
\end{figure}
...
Así, el área debajo de la parábola que pasa por los puntos $(x_{2},f(x_{2}))$, $(x_{3},f(x_{3}))$ y $(x_{4},f(x_{4}))$ es $A_{2}=\frac{h}{3}[f(x_{2})+4f(x_{3})+f(x_{4})]$; entonces, por los puntos $(x_{4},f(x_{4}))$, $(x_{5},f(x_{5}))$ y $(x_{6},f(x_{6}))$, es $A_{3}=\frac{h}{3}[f(x_{4})+4f(x_{5})+f(x_{6})]$; y, $\ldots$, por los puntos $(x_{n-2},f(x_{n-2}))$, $(x_{n-1},f(x_{n-1}))$ y $(x_{n},f(x_{n}))$ es $A_{n}=\frac{h}{3}[f(x_{n-2})+4f(x_{n-1})+f(x_{n})]$, lo que, sumando todas las áreas de las parábolas, da como resultado
\begin{gather*}
  \begin{aligned}
  \int_{a}^{b}&f(x)\,dx \approx S_{n}=\frac{h}{3}\left[f(x_{0})+4f(x_{1})+f(x_{2})\right]\\
  &\quad+\frac{h}{3}\left[f(x_{2})+4f(x_{3})+f(x_{4})\right]+\frac{h}{3}\left[f(x_{4})+4f(x_{5})+f(x_{6})\right] +\ldots \\
  &\quad+\frac{h}{3}\left[ f(x_{n-2})+4f(x_{n-1})+f(x_{n})\right] \\
  &=\frac{h}{3}\left[f(x_{0})+4f(x_{1})+2f(x_{2})+4f(x_{3})\right.\\
  &\quad\left.+f(x_{4})+4f(x_{5})+f(x_{6})+\ldots+f(x_{n-2})+4f(x_{n-1})+f(x_{n})\right] \\
  &=\frac{h}{3}\left[f(x_{0})+4f(x_{1})+2f(x_{2})+4f(x_{3})\right.\\
    &\quad\left. +2f(x_{4})+4f(x_{5})+2f(x_{6})+\ldots+2f(x_{n-2})+4f(x_{n-1})+f(x_{n})\right]
  \end{aligned}
\end{gather*}

En resumen, para aproximar el valor de $\int_{a}^{b}f(x)\,dx$ por el método de Simp\-son, se usa la expresión
...
\]
con $h=\Delta x=\frac{b-a}{n}$ y $n$ un número par.

\subsubsection{Error en el método de Simpson}

Si $f^{\left( 4\right) }$ es continua y $M$ es una cota superior de $\left\vert f^{\left( 4\right) }\right\vert$ en $\left[ a,b\right]$,
...
\item $\int_{1}^{2}e^{1/x}dx$ con $n=4$.
  ...
  \emph{Solución.} En este caso tenemos que $\Delta x=\frac{2-1}{4}=\frac{1}{4}$ \ y
  \begin{align*}
    P&=\{1,1+1/4,1+2(1/4),1+3(1/4),1+4(1/4)\}\\
    &=\{1,5/4,3/2,7/4,2\}
  \end{align*}
  ...
  \begin{gather*}
    \begin{aligned}
      \int_{1}^{2}e^{1/x}dx&\approx S_{n}=\frac{1/4}{3}
      \Big[f(1)+4f(5/4)+2f(3/2)+4f(7/4)+f(2)\Big]\\
      &=\frac{1}{12}\Big[ e^{1/1}+4e^{4/5}+2e^{2/3}+4e^{4/7}+e^{1/2}\Big]\\
      &=0.0833(2.7183+8.9022+3.8955+7.0832+1.6487)\\
      &=0.0833(24.248)\approx 2.0199
    \end{aligned}
  \end{gather*}

\item $\int_{0}^{3}\frac{1}{1+x^{4}}dx$ con $n=6$.

  \emph{Solución.} Ahora, $\Delta x=\frac{3-0}{6}=\frac{3}{6}=\frac{1}{2}$ \ y
  \begin{align*}
  ...
  \begin{gather*}
    \begin{aligned}
      ...
      &=(0.1666)(6.6542) \approx 1.106
    \end{aligned}
  \end{gather*}

\item Usando el método de Simpson, halle el área de la región dada en la figura~\ref{fig:InteNumSimpsEjemp}:

  \begin{figure}[H]
    \centering
    \caption{Región por medir}\label{fig:InteNumSimpsEjemp}
    \includegraphics[width=8cm]{Gráficas/InteNumSimpsEjemp}
    \propia
  \end{figure}
...
Decida si la integral es convergente o divergente. En caso de ser convergente, calcule la integral impropia dada:

\begin{multicols}{3}
  \exercise{$\int_{1}^{\infty }\frac{1}{x^{0.99}}dx$} {La integral diverge}

  \exercise{$\int_{1}^{\infty }e^{-4x}dx$} {Converge a $\frac{1}{4e^{4}}$}

  \exercise{$\int_{-\infty }^{3}e^{2x}dx$} {Converge a $\frac{1}{2}e^{6}$}

  \exercise{$\int_{-\infty }^{\infty }e^{-x}dx$} {La integral diverge}

  \exercise{$\int_{e}^{\infty }\frac{1}{x\left( \ln x\right) ^{3}}dx$} {Converge a $\frac{1}{2}$}

  \exercise{$\int_{0}^{\infty }e^{-x}\sin x\,dx$} {Converge a $\frac{1}{2}$}

  \exercise{$\int_{0}^{2}\frac{1}{\sqrt[3]{x-1}}dx$} {Converge a $0$}

  \exercise{$\int_{0}^{\frac{\pi }{4}}\frac{\sec ^{2}\theta\,d\theta }{\sqrt{\tan\theta }}$} {Converge a $2$}

  \exercise{$ \int_{2}^{\infty }\frac{1}{\left( x+1\right) \left( x+2\right) }dx$} {Converge a $\ln \frac{4}{3}$}

  \exercise{$\int_{1}^{\infty }\frac{1}{x}dx$} {La integral diverge}

  \exercise{$\int_{2}^{\infty }\frac{2}{\left( x+1\right) ^{2}}dx$} {Converge a $\frac{2}{3}$}

  \exercise{$\int_{1}^{\infty }\frac{8}{x^{3}+x}dx$} {Converge a $4\ln 2$}

  \exercise{$\int_{2}^{\infty }\frac{x}{x^{4}-1}dx$} {Converge a $\frac{1}{4}\ln \frac{5}{3}$}

  \exercise{$\int_{3}^{\infty }\frac{x}{x^{4}+3x^{2}-4}dx$} {Converge a $\frac{1}{10}\ln \frac{13}{8}$}

  \exercise{$\int_{1}^{\infty }\frac{dx}{e^{x+2}+e^{4-x}}$} {Converge a $\frac{\pi }{4e^{3}}$}

  \exercise{$\int_{2}^{\infty }\frac{x^{2}}{\left( x^{2}+4\right) ^{2}}dx$} {Converge a $\frac{1}{16}\pi +\frac{1}{8}$}

  \exercise{$\int_{1}^{\infty }\frac{4}{x\left( x+1\right) ^{2}}dx$} {Converge a $4\ln 2 -2$}

  \exercise{$\int_{3}^{\infty }\frac{x}{\left( x-1\right) ^{2}\left( x+1\right) }dx$} {Converge a $\frac{1}{4}\ln 2+\frac{1}{4}$}

  \exercise{$\int_{2}^{\infty }\frac{1}{x^{2}-1}dx$} {Converge a $\frac{1}{2}\ln 3$}

  \exercise{$\int_{2}^{\infty }\frac{dx}{x\left( x-1\right) \left( x+1\right) }$} {Converge a $\frac{1}{2}\ln \frac{4}{3}$}

  \exercise{$\int_{2}^{\infty }\frac{10}{\left( x+2\right) \left( x+3\right)}dx $} {Converge a $10\ln 5-20\ln 2 $}

  \exercise{$\int_{1}^{\infty }\frac{10}{x^{3}+4x}dx$} {Converge a $\frac{5}{4}\ln 5 $}

  \exercise{$\int_{-\infty }^{\infty }\frac{3x^{2}dx}{x^{6}+1}$} {Converge a $\pi$}

  \exercise{$\int_{-2}^{3}\frac{1}{x^{3}}dx$} {La integral diverge}

  \exercise{$\int_{-1}^{8}\frac{1}{\sqrt[3]{x}}dx$} {Converge a $\frac{9}{2}$}

  \exercise{$\int_{0}^{1}x\ln x\,dx$} {Converge a $-\frac{1}{4}$}

  \exercise{$\int_{0}^{\infty }\frac{1}{\left( 1+x\right) \sqrt{x}}dx$} {Converge a $\pi$}

  \exercise{$\int_{0}^{e}\ln ^{2}z\,dz$} {Converge a $e$}
\end{multicols}

\exercise{\textls[-10]{Aproxime las siguientes integrales usando el método del punto medio, la regla del trapecio y el método de Simpson, con el valor de $n$ indicado:}}
%\begin{multicols}{2}
\subexercise{$\int_{2}^{4}\frac{x}{\ln x}dx$  con $n=6$}{$M_{6}=5.5483,\ T_{6}=5.5598,\ S_{6}=5.5525$}

\subexercise{$\int_{0}^{1}\frac{e^{x}}{1+x^{2}}dx$ con $n=10$}{$ M_{10}=1.2711,\ T_{10}=1.2699,\ S_{10}=1.2707$}

\subexercise{$\int_{4}^{6}\ln \left( x^{3}+2\right) dx$ con $n=10$}{$M_{10}=9.6509,\ T_{10}=9.6498,\ S_{10}=9.6505$}

\subexercise{$\int_{2}^{10}\frac{x^{2}+10x}{\ln x}dx$ con $n=10$}{$ M_{10}=443.59,\ T_{10}=444.66,\ S_{10}=444.01$}

\subexercise{$\int_{0}^{3}\sqrt{x^{6}+2}dx$ con $n=8$}{$%
M_{10}=21.706$, $T_{10}=22.180$, $S_{10}=21.863$}
%\end{multicols}

\exercise{Dada la figura~\ref{fig:ejempintnum}, aproxime el área abajo de la región sombreada para $n=6$ utilizando:}

\subexercise{El método del punto medio.}{$M_{6}=24.6$ }

\subexercise{La regla del trapecio.}{ $T_{6}=24.1$}

\subexercise{El método de Simpson.}{$S_{6}=24.4$}

\begin{figure}[H]
  \centering
  \caption{Área para aproximar}
  \label{fig:ejempintnum}
  \includegraphics[width=6cm]{Gráficas/ejempintnum.pdf}
  \propia
\end{figure}

\exercise{Sea $f$\ la función cuyos datos están en la siguiente tabla:
  \begin{center}
    \begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|}
      \hline
      $x$ & $0$ & $0.5$ & $1$ & $1.5$ & $2$ & $2.5$ & $3$ & $3.5$ & $4$ & $4.5$ & $5$ \\ \hline
      $f\left( x\right) $ & $0.8$ & $1$ & $1.6$ & $2$ & $3$ & $3.8$ & $3.2$ & $2.6$ & $2.2$ & $1.8$ & $1.5$ \\ \hline
    \end{tabular}
  \end{center}
}

\subexercise{Use la tabla para aproximar $\int_{0}^{5}f\left( x\right) dx$ usando el método del punto medio.}{$M_{5}=11.2$}

\subexercise{Si $-3\leq f^{\prime \prime}\left( x\right) \leq 4$ para todo $x$, estime el error en la aproximación de la parte a).}{Error $0.8333$}

\exercise{Utilice los datos de la función $f$ dados en la siguiente tabla:
  \begin{center}
    \begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|}
      \hline
      $x$ & $2$ & $2.5$ & $3$ & $3.5$ & $4$ & $4.5$ & $5$ & $5.5$ & $6$ & $6.5$ & $7$ \\ \hline
      $f\left( x\right) $ & $1$ & $1.8$ & $2.6$ & $3.8$ & $4.8$ & $6$ & $5.2$ & $4$ & $3.8$ & $3.2$ & $3$ \\
      \hline
    \end{tabular}
\end{center}}

\subexercise{Para aproximar $\int_{2}^{7}f\left( x\right) dx$ usando la regla del trapecio.}{$T_{10}=18.6$ }

\subexercise{Si $-4\leq f^{\prime \prime}\left( x\right) \leq 6$ para todo $x\in \left[ 2,7\right]$, estime el error en la aproximación de la parte a).}{Error $0.625$}
...
\begin{center}
  \begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|c|}
    \hline
    $x$ & $4$ & $4.5$ & $5$ & $5.5$ & $6$ & $6.5$ & $7$ & $7.5$ & $8$ & $8.5$ & $9$ & $9.5$ & $10$ \\ \hline
    $f\left( x\right) $ & $3$ & $3.8$ & $4$ & $4.6$ & $5$ & $5.4$ & $6.6$ &
    $5.8$ & $5$ & $4$ & $3.6$ & $3$ & $2.4$ \\ \hline
  \end{tabular}
\end{center}}

\subexercise{Para aproximar $\int_{4}^{10}f\left( x\right) dx$ usando la regla del trapecio.}{$S_{12}=26.7$}

\subexercise{Si $-5\leq f^{\left( 4\right) }\left( x\right) \leq 10$ para todo $x\in \left[ 4,10\right] $, estime el error en la aproximación de la parte a).}{Error $0.00837$}

\exercise{}

\subexercise{Halle $n$ para que $\int_{0}^{1}e^{\frac{1}{4}x^{2}}dx$ tenga magnitud de error menor que $10^{-3},$ usando la regla del trapecio, y halle $T_{n}$.}{$n=10$\qquad $T_{10}=1.0905$}

\subexercise{Halle $n$ para que $\int_{0}^{1}e^{\frac{1}{4}x^{2}}dx$ tenga magnitud de error menor que $10^{-4},$ usando el método de Simpson, y halle $S_{n}$.}{$n=4$ \qquad $S_{4}=1.09$}
