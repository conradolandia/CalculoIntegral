...
\begin{enumerate}
  ...
  \item {$v(t)=t^{3}+t^{2}+t,\ s\left(0\right)=3$}{$s(t) =\frac{t^{4}}{4}+\frac{t^{3}}{3}+\frac{t^{2}}{2}+3$}

  \item {$v(t)=\sen t+\cos t,\ s\left(\pi\right)=2$}{$s(t) =-\cos (t) +\sen (t)
    +1$}

  \item {$v(t)=\sec ^{2}t+\csc ^{2}t,\
    s\left( \frac{\pi }{4}\right) =2$}{$s(t) =\tan (t)
    -\cot (t) +2$}

  \item {$v(t)=\cfrac{1-\cos ^{2}t}{\sen ^{2}t},\ s\left( \frac{\pi }{4}\right) =8$}{$s(t) =t+8-\frac{\pi
    }{4}$}
\end{enumerate}
\end{ejer}
...
\begin{enumerate}
  ...
  \item {$a(t)=t^{3}+t^{2},\ v\left( 0\right)
    =3,s\left( 0\right) =5$}{$s(t) =\frac{t^{5}}{20}+\frac{t^{4}}{12}+3t+5$}

  \item {$a(t)=\sen t+\cos t,\ v(\pi )=4,$ $\
    s\left( \pi \right) =2$}{$s(t) =-\sen (t) -\cos
    (t) +3t-3\pi +1$}

  \item {$a(t)=\cfrac{2t^{3}+t^{2}}{t^{5}},\
    v\left( 1\right) =4,s\left( 1\right) =10$}{$s(t) =-2\ln\left\vert t\right\vert+\frac{1}{2t}+\frac{13}{4}t^{2}+\frac{25}{4}$}

  \item {$a(t)=-9.8t,\ v(2)=7,$ $\ s\left(3\right) =9\,$}{$s(t) =-\frac{9.8}{6}t^{3}+26,6t-26,7$}
\end{enumerate}
\end{ejer}
%\clearpage
...
\begin{enumerate}
  ...
  \item {$f^{\prime\prime}(x) =e^{x}-x,\ f\left( 1\right) =3,\ f'\left( 2\right) =1$}{$f(x)=e^{x}-\frac{x^{3}}{6}
    +(3-e^{2})x-\frac{6e-1-6e^{2}}{6}$}

  \item {$f^{\prime\prime\prime}(x) =\sen x-\cos\left( 2x\right),\ f\left( 0\right) =f'\left( \frac{\pi }{2}
    \right) =f^{\prime\prime}\left( \pi \right) =1$}{$f(x) =\cos x+\frac{\sin \left( 2x\right) }{8}+\frac{9}{4}x$}

  \item {$\cfrac{d^{\,2}y}{dx^{2}}=2x^{2}+3x^{3},\ y(1)=3,y'\left(2\right)=3$}{$y=\frac{x^{4}}{6}+\frac{3x^{5}}{20}-\frac{43}{3}x+\frac{1021}{60}$}

  \item {$\cfrac{d^{\,4}y}{dx^{4}}=120x^{2},\ y\left( 0\right) =y'\left( 0\right) =y^{\prime\prime}\left( 0\right)=y^{\prime\prime\prime}\left( 1\right) =1$}{$ y=\frac{1}{3}x^6 -\frac{13}{2}x^{3}+\frac{1}{2}x^{2}+x+1$}
\end{enumerate}
\end{ejer}
...
segundos, su velocidad inicial es 6 metros por segundo y su posición inicial es 4 metros.\ Halle la función de posición de la partícula.}{$s(t) =-\frac{\sen \left( 2t\right) }{4}+\frac{e^{2t}}{4}+\frac{2^{4t}}{16\ln ^{2}\left( 2\right) }+\left( 6-\frac{1}{4\ln \left(2\right) }\right) t+\frac{15}{4}-\frac{1}{16\ln ^{2}\left( 2\right) }$}
\end{ejer}

\begin{ejer}{Si la pendiente de la tangente a la gráfica de$f$ en $\left(x,f(x) \right)$ \ es $\cfrac{\cos  ^{2}x}{1+\sen x}$ y $f\left( \frac{\pi }{6}\right) =4$, halle la función
$f(x).$}{$f(x) =x+\cos x+4-\frac{\pi}{6}-\frac{\sqrt{3}}{2}$}
\end{ejer}
...
\begin{enumerate}
  ...
  \item {$f(x) =6x^{2}+4x+4$ en $\left[1,3\right].$}{$c=\frac{2}{3}\sqrt{13}-\frac{1}{3}$}

  \item {$f(x) =x^{3}$ en $\left[ 2,4\right] .$}{$c=\sqrt[3]{30}=3,1072$}

  \item {$f(x) =3^{x}$ en $\left[ 0,2\right] $.}{$c=\frac{\ln \left( \frac{4}{\ln 3}\right) }{\ln 3}=1.1763$}

  \item {$f(x) =2^{3x}$ en $\left[ 0,4\right] $.}{$c=\frac{\ln \left( \frac{1365}{4\ln 2}\right) }{\ln 8}=2,9812$}

  \item {$f(x) =2\sen x$ en $\left[ 0,\frac{\pi }{2}\right] $.}{$c=\arcsin \left( \frac{2}{\pi }\right) =0,69011$}

  \item {$f(x) =4\cos x$ en $\left[ 0,\frac{\pi }{2}\right] $.}{$c=\arccos \left( \frac{2}{\pi }\right)=0,88069$}
    %\end{multicols}
\end{enumerate}
\end{ejer}
%\clearpage

Para cada una de las funciones $f(x)$ dadas en los siguientes ejercicios: a) calcule la función $A(x) =\int_{a}^{x}f(t)dt$, de área de acumulación de $f(x)$ a partir del valor de $t=a$ indicado; b) halle la derivada respecto a $x$ de la funcíon área de acumulación $\cfrac{dA}{dx}$.

\begin{ejer}
{$f(x) =3x^{2}+6x+2$ a partir de $t=2$} \\
{$a)$ $A(x) =x^{3}+3x^{2}+2x-24\qquad \\
b)$ $\frac{dA}{dx}
=3x^{2}+6x+2=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =4x^{3}+3x^{2}+1$ a partir de $t=3$} \\
{$a)$ $A(x) =x^{3}+3x^{2}+2x-24\qquad \\
b)$ $\frac{dA}{dx} =3x^{2}+6x+2=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =12x^{5}+6x^{2}+2$ a partir de $t=1$} \\
{$a)$ $A(x) =2x^{6}+2x^{3}+2x-6\qquad \\ b)$ $ \frac{dA}{dx}=12x^{5}+6x^{2}+2=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =e^{x}+2x$ a partir de $t=0$} \\
{$a)$ $A(x) =e^{x}+x^{2}-1\qquad \\ b)$ $\frac{dA}{dx}=e^{x}+2x=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =2^{x}+3^{x}+8x+1$ a partir de $t=0$} \\
{$a)$ $A(x) =\frac{2^{x}}{\ln 2}+\frac{3^{x}}{\ln 3}+4x^{2}+x-\frac{1}{\ln 2}-\frac{1}{\ln 3}\qquad \\
b)$ $\frac{dA}{dx}
=2^{x}+3^{x}+8x+1=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =2\sen x+3x$ a partir de $t=0$} \\
{$a)$ $A(x) =\frac{3}{2}x^{2}-2\cos x+2\qquad \\
b)$ $\frac{dA}{dx}=2\sen x+3x=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =e^{2x}+\sen x+1$ a partir de $t=0$} \\
{$a)$ $A(x) =x-\cos x+\frac{1}{2}e^{2x}+\frac{1}{2}\qquad \\
b)$ $e^{2x}+\sen x+1=f(x)$}
\end{ejer}

\begin{ejer}
{$f(x) =e^{4x}+\sen x+\cos x$ a partir de $t=0$} \\
{$a)$ $A(x) =\sen x-\cos x+\frac{1}{4}e^{4x}+\frac{3}{4}\qquad \\
b)$ $\frac{dA}{dx}=e^{4x}+\sen x+\cos x=f(x)$}
\end{ejer}
...
\item {$\int_{a}^{x}\frac{f(t) }{t^{2}}dt=3x^{2}-9$}{$f(x)=6x^{3}\qquad a=\pm \sqrt{3}$}

\item {$\int_{a}^{x}\frac{f(t)+1}{f(t)-1}dt=\frac{x-1}{2x+1}$}{$f(x) =\frac{-3-\left( 2x+1\right) ^{2}}{\left( 2x+1\right)
^{2}-3}\qquad a=1$}
