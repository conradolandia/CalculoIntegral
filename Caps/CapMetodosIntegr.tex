...
\begin{defi}
  Sea $y=f(x)$ una función diferenciable. La diferencial $dx$ es una variable independiente y la diferencial $dy$ es igual a $f^{\prime}(x)dx$.
\end{defi}
...
Para calcular una integral que tenga la forma $\int f(g(x))g^{\prime}(x)dx$, es decir, una integral en la que se tenga una composición de dos funciones y la derivada de la función interna (salvo, quizás, constantes) multiplique a esa composición\footnote{El hecho de que se tenga $g^\prime(x)$ no es completamente estricto. Puede tenerse una expresión muy similar a la derivada de $g$ de tal manera que la sustitución también funcione.}, puede hacerse la sustitución
...
\begin{ejemplo}
  Si se quiere calcular la integral $\displaystyle{\int e^{x^2}xdx}$, donde el $x$ que multiplica a la exponencial es la derivada (salvo el $2$) de $x^2$, entonces podemos hacer la sustitución.
\end{ejemplo}
\[
...
\begin{ejemplo}
  Encuentre el valor de la integral $\displaystyle{\int_{2}^{17}\frac{x}{\sqrt[4]{x-1}}\,dx}$
\end{ejemplo}
...
\begin{ejemplo}
  Calcule $\displaystyle{\int \left( 1+\sen x\right) ^{2}\cos xdx}$.
\end{ejemplo}

\emph{Solución.} Si $u=1+\sen x$, $du=\cos xdx$:
{\small
  \begin{align*}
...
\begin{ejemplo}
  Halle $\displaystyle{\int x\sqrt[3]{x+1}dx}$.
\end{ejemplo}
...
\begin{ejemplo}
  Encuentre $\displaystyle{\int_{0}^{\frac{\pi }{4}}\sec ^{4}\theta \tan ^{3}\theta d\theta}$.
\end{ejemplo}
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.45\textwidth]{Gráficas/GrafArcsec.pdf}
  \propia
\end{figure}
...
\begin{ejemplo}
Calcule $\displaystyle{\int_{-\pi }^{\pi }\frac{\sen ^{7}\left( x^{3}\right) }{x^{4}+1}dx}$.
\end{ejemplo}
...
\emph{Solución.} Si en $\int_{-2}^{2}x^{4}f\left( x^{5}\right) dx$ hacemos la sustitución $\ u=x^{5}\rightarrow du=5x^{4}dx\rightarrow x^{4}dx=\frac{1}{5}du$ y cambiamos límites de integración. Si $x=-2$, entonces $u=\left( -2\right)^{5}=-32$ y, si $x=2$, entonces $u=2^{5}=32$.
\[
...
\subsection*{Integrales de la forma $\int P(x)f(x)dx$}\label{ProdFunciones}
...
\includegraphics[width=0.70\textwidth]{Gráficas/TablaPartes.pdf}
...
\begin{ejemplo}
Tomemos $\displaystyle{\int (x^{3}+x^{2})\ln xdx}$.
\end{ejemplo}
...
\begin{ejemplo}
La integral $\displaystyle{\int \ln xdx}$ puede incluirse en el caso anterior.
\end{ejemplo}
...
\begin{ejemplo}
Tomemos $\displaystyle{\int \ln ^{3}xdx }$.
\end{ejemplo}
...
\end{ejemplo}
\emph{Solución.} Nuevamente hacemos la sustitución que elimine, para este caso, la función trigonométrica inversa: si $y=\sen^{-1} x$, entonces $\sen y= x$ y $\cos ydy= dx$ y, por tanto, $\sen^{-1} xdx=y\cos ydy$. Por otro lado, construyendo la tabla de partes se tiene
\renewcommand{\arraystretch}{1.5}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
...
\end{ejemplo}
\emph{Solución.} De manera similar a los ejemplos anteriores, si $y=\tan^{-1} x$, entonces $\tan y= x$ y $\sec^2 ydy= dx$; por tanto, $\tan^{-1} xdx=y\sec^2 ydy$.
...
\end{ejemplo}
\emph{Solución.} Si $y=\sin^{-1} x$, entonces $\sin y= x$ y $\cos ydy= dx$; por tanto, $(\sin^{-1} x)^2dx=y^2\cos ydy$ y la tabla
...
\begin{ejemplo}
Una función que suele generar confusión para integrar es $\sec^3\theta$. A continuación se muestra cuál es el manejo de {\small $\displaystyle{\int \sec ^{3}\theta d\theta }$.}
\end{ejemplo}

\emph{Solución.} Para iniciar, hagamos que $S=\displaystyle{\int \sec ^{3}\theta d\theta=\int \sec\theta \sec^{2}\theta d\theta}$. Al aplicar partes, tenemos
\begin{align*}
...
\begin{ejemplo}
Encontremos el valor de $\displaystyle{\int e^{4x}\cos (2x)dx}$.
\end{ejemplo}
...
\begin{ejemplo}
Tomemos, por ejemplo, $\displaystyle{\int \sen ^{n}xdx}$ para $n$ un número entero y usemos integración por partes.
\end{ejemplo}
\emph{Solución}
{\small
\begin{align*}
...
\begin{ejemplo}
Algo similar pasa con $\displaystyle{\int \cos^{n}xdx}$.
\end{ejemplo}
\emph{Solución}
{\small
\begin{align*}
...
\begin{ejemplo}
Tomemos ahora $\displaystyle{ \int \sec ^{n}\theta d\theta}$.
\end{ejemplo}
\emph{Solución.}
Hacemos
{\small
...
\begin{ejemplo}
Tomemos ahora potencias de $a+x^{2}$, con $a\in\mathbb{R}$, es decir, $\displaystyle{\int \left( a+x^{2}\right)^{n}dx }$.
\end{ejemplo}
\emph{Solución.} Supongamos que $I=\displaystyle{\int \left( a+x^{2}\right)^{n}dx }$ y hacemos
\begin{align*}
...
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{x}{\sqrt{25-9(x+3)^{2}}}dx}$
\end{ejemplo}
...
\includegraphics[width=0.37\textwidth]{Gráficas/TriangSustSeno5_3.pdf}
...
\begin{ejemplo}
Evalúe la integral $\displaystyle{\int \frac{1-x}{-20-4x^{2}+24x}dx}$.
\end{ejemplo}
...
Así; entonces, tomando la sustitución $x-3=2\sen(\theta ),$ con $\frac{-\pi }{2}< \theta < \frac{\pi }{2}$ y $dx=2\cos (\theta
...
\includegraphics[width=0.34\textwidth]{Gráficas/TriangSustSeno.pdf}
...
\begin{ejemplo} Calcule $\displaystyle{\int \frac{\sqrt{16-x^{2}}}{x}dx}$.
\end{ejemplo}
...
\centering
\caption{Triángulo en la sustitución}
\includegraphics[width=0.28\textwidth]{Gráficas/TriangSustSeno2.pdf}
...
\begin{ejemplo}
Calcule $\displaystyle{\int_{0}^{\ln \sqrt{2}}\frac{e^{z}dz}{\left( 4-e^{2z}\right) ^{\frac{3}{2}}}}$.
\end{ejemplo}
...
\begin{ejemplo} Encontremos el valor de $\displaystyle{\int\frac{\sqrt{4+x^{2}}}{x}dx}$.
\end{ejemplo}
\emph{Solución.} Si $x=2\tan \theta $ para $\theta\in \left(-\frac{\pi}{2},\frac{\pi}{2}\right)-\{0\}$, entonces $dx=2\sec ^{2}\theta d\theta $ y
{\small
  \begin{align*}
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.28\textwidth]{Gráficas/SustTrigonTang.pdf}
  \propia
\end{figure}
...
\begin{ejemplo} Hallemos el valor de $\displaystyle{\int_{0}^{1}\frac{x^{2}}{\sqrt{1+x^{2}}^{\,3}}dx}$.
\end{ejemplo}
...
\begin{ejemplo} Encontremos el valor de $\displaystyle{\int\frac{dx}{\sqrt{x^{2}-9}}}$.
\end{ejemplo}
...
\begin{ejemplo} Encontremos el valor de {\small $\displaystyle{\int e^{t} \sqrt{e^{2t}-100}dt}$.}
\end{ejemplo}

\emph{Solución.} Si $e^{t}=10\sec \theta $ para $0\leq \theta <\pi/2$, entonces $e^{t}dt=10\sec\theta\tan\theta d\theta $ y
{\small
  \begin{align*}
...
\centering
\caption{Triángulo con los datos de la sustitución}
\includegraphics[width=0.35\textwidth]{Gráficas/SustTrigonSec2.pdf}
...
\begin{ejemplo}
Encontremos el valor de $\displaystyle{\int_{1}^{\sqrt{2}}\frac{\sqrt{x^{2}-1}}{x}dx}$.
\end{ejemplo}
...
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{x^{2}dx}{(x-1)(x+1)(x-2)}}$.
\end{ejemplo}
...
\begin{ejemplo}
Calcular $\int_{0}^{1}\frac{2x^{2}+10x+14}{\left( x+1\right) \left(
...
\]
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{x^{3}}{(x-2)^{4}}dx}.$
\end{ejemplo}
\emph{Solución.} La descomposición en fracciones parciales toma la siguiente forma:
\begin{align*}
...
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{x+1}{(x^{2}+1)(x^{2}+4)}dx}$.
\end{ejemplo}
...
Ahora bien, las integrales anteriores deben solucionarse haciendo algunas sustituciones. En su orden, la primera integral se soluciona con la sustitución $u=x^{2}+1$, de donde $du=2xdx$ y $\frac{du}{2}=xdx$. Y la tercera integral con la sustitución $z=x^{2}+4,$ $dz=2xdx$ y, así, $\frac{dz}{2}=xdx$.
...
\end{ejemplo}
\emph{Solución.} Tomemos inicialmente la sustitución {\small $y=\sqrt{x}\rightarrow y^{2}=x\rightarrow 2ydy=dx$,} entonces
{\small
\[
...
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{dx}{\sqrt{x}+\sqrt[4]{x}-2}}$.
\end{ejemplo}
...
Las expresiones de la forma {\small $\int \frac{P(\sen x,\cos x)dx}{Q(\sen x,\cos x)}$} donde $P$ y $Q$ son polinomios se llaman una \textit{función racional de seno y coseno}.
...
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{dx}{3\sen x+4\cos x+5}}$.
\end{ejemplo}
...
\begin{ejemplo}
Calcule $\displaystyle{\int \frac{dx}{4\sen x+\cos x+4}}.$
\end{ejemplo}
...
Para la integral $\int \left( \frac{3}{u}-\frac{3}{u+\frac{1}{3}}\right)\, du$ de arriba, se usó, al igual que en las anteriores, descomposición en fracciones parciales para $\cfrac{1}{u\left( u+\frac{1}{3}\right)}$:
\[
...
Calcule cada una de las siguientes integrales. Use una sustitución adecuada.\\

{\small
\begin{enumerate}
    %\begin{multicols}{2}

  \item {$\int_{0}^{2}\left( 4y+1\right) ^{100}dy$}{ $\frac{9^{101}-1}{404}.$}

  \item {$\int_{2}^{5}t(t^{2}+1)^{10}dt$}{$\frac{26^{11}-5^{11}}{22}$}

  \item {$\int x\left( \cos x^{2}+\sin x^{2}\right) dx$}{$\frac{1}{2}\left( \sin \left( x^{2}\right) -\cos \left( x^{2}\right) \right) +C$}

  \item {$\int z\sqrt{4-z^{2}}dz$}{$-\frac{1}{3}\sqrt{4-z^{2}}^{3}+C$}

  \item {$\int_{0}^{3}\frac{w}{\sqrt{25-w^{2}}}dw$}{$1$}

  \item {$\int \frac{e^{x}dx}{\sqrt{e^{x}-1}}$}{$2\sqrt{e^{x}-1}+C$}

  \item {$\int t(t+1)^{4}dt$}{$ \frac{\left( t+1\right) ^{6}}{6}-\frac{\left( t+1\right) ^{5}}{5}+C$}

  \item {$\int y\sqrt{y-1}dy$}{$2\left( \frac{\sqrt{y-1}^{5}}{5}+\frac{\sqrt{y-1}^{3}}{3}\right) +C$}

  \item {$\int \frac{2y}{\sqrt[4]{3y-2}}dy$}{$\frac{8}{9}\left(\frac{\sqrt[4]{3y-2}^{7}}{7}+2\frac{\sqrt[4]{3y-2}^{3}}{3}\right) +C$}

  \item {$\int 2x\left\vert x\right\vert dx$}{$\frac{2x^{2}\left\vert x\right\vert }{3}+C$}

  \item {$\int \frac{\sin t}{\sqrt{4+5\cos t}}dt$}{$ -\frac{2}{5}\sqrt{4+5\cos t}+C$}

  \item {$\int \sin ^{5}ydy$}{$\frac{-15\cos y+10\cos ^{3}y-3\cos ^{5}y}{15}+C$}

  \item {$\int_{0}^{\frac{\pi }{2}}\cos ^{7}zdz$}{$\frac{16}{35}$}

  \item {$\int_{0}^{\frac{\pi }{4}}\tan ^{8}\theta d\theta$}{$\frac{1}{4}\pi -\frac{76}{105}$}

  \item {$\int \cot ^{10}\theta d\theta$}{$-\frac{1}{9}\cot ^{9}\theta$ \linebreak $+\frac{1}{7}\cot ^{7}\theta -\frac{1}{5}\cot ^{5}\theta +\frac{1}{3}\cot^{3}\theta -\cot \theta -\theta +C$}

  \item {$\int \sec ^{6}tdt$}{$\tan t+\frac{2\tan ^{3}t}{3}+\frac{\tan  ^{5}t}{5}+C$}

  \item {$\int \csc ^{4}xdx$}{$-\cot \left( x\right) -\frac{\cot  ^{3}\left( x\right) }{3}+C$}

  \item {$\int \cos ^{6}(2y)dy$}{$\frac{1}{12}\cos ^{5}\left( 2y\right) \sin \left( 2y\right) -\frac{5}{48}%
      \cos ^{3}\left( 2y\right) \sin \left( 2y\right) -\frac{5}{32}\cos \left(
    2y\right) \sin \left( 2y\right) +\frac{5}{16}y+C$}

  \item {$\int \sin ^{6}(z)dz$}{$-\frac{1}{6}\sin ^{5}z\cos z-\frac{5}{24}\sin ^{3}z\cos z-\frac{5}{16}\sin
    z\cos z+\frac{5}{16}z+C$}

  \item {$\int \tan ^{5}y\sec ^{4}ydy$}{$\frac{\tan ^{6}\left( y\right)}{6}+\frac{\tan ^{7}\left( y\right) }{7}+C$}

  \item {$\int \tan ^{7}\left( 7y\right) dy$}{$\frac{1}{7}\left( \frac{\left( \tan \left( 7y\right) \right) ^{6}}{6}-\frac{%
        \left( \tan \left( 7y\right) \right) ^{4}}{4}+\frac{\left( \tan \left(
        7y\right) \right) ^{2}}{2}-\ln \left\vert \sec \left( 7y\right) \right\vert
    \right) +C$}

  \item {$\int \cot ^{5}xdx$}{$-\frac{1}{4}\cot ^{4}x+\frac{1}{2}\cot^{2}x+\ln \left\vert \sin x\right\vert +C$}

  \item {$\int \sin ^{4}x\cos ^{5}xdx$}{$\frac{\sin ^{5}\left( x\right)}{5}-\frac{2\sin ^{7}\left( x\right) }{7}+\frac{\sin ^{9}\left( x\right) }{9}+C$}

  \item {$\int \cot ^{3}y\csc ^{4}ydy$}{$-\left( \frac{\cot ^{6}\left( y\right) }{6}+\frac{\cot ^{4}\left( y\right)
    }{4}\right) +C$}

  \item {$\int_{\frac{1}{2}}^{1}\frac{1}{x^{2}}\sqrt{1+\frac{1}{x}}dx$}{$2\sqrt{3}-\frac{4}{3}\sqrt{2}$}

  \item {$\int_{\frac{\pi ^{2}}{16}}^{\frac{\pi ^{2}}{4}}\frac{\sin (\sqrt{y})}{\sqrt{y}}dy$}{$\sqrt{2}$}

  \item {$\int \frac{xdx}{\sqrt[3]{x}+\sqrt{x}}$}{$-\frac{2\sqrt{x}}{3}-\frac{2\sqrt[3]{x}}{3}-\frac{8}{9}\sqrt[6]{x}-\frac{16%
    }{27}\ln \left\vert -3\sqrt[6]{x}+2\right\vert +C$}

  \item {$\int \frac{dx}{2\sqrt[3]{x}-3\sqrt{x}}$}{$
      -\frac{8}{9}\sqrt[6]{x}-\frac{2}{3}\sqrt[3]{x}-\frac{2}{3}\sqrt{x}-\frac{16}{27}\ln
    \left\vert \sqrt[3]{u^{6}}-\frac{2}{3}\right\vert +C$}

    %\exercise{$\int \cos ^{2}\left( \sin \left( x\right) \right) \cos \left( x\right)dx$}{$\frac{1}{2}\left( \sin \left( x\right) +\sin \left(\sin \left( x\right) \right) \cos \left( \sin \left( x\right) \right)\right) +C$}
    %\end{multicols}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item {$\displaystyle{\int \frac{3x^{2}}{(x-1)(x+2)(x+3)}\, dx}$}{ $\frac{1}{4}\ln \left\vert x-1\right\vert -4\ln \left\vert x+2\right\vert +
    ...
    %%\begin{multicols}{2}
  \item {$\int \frac{dx}{\sen x-2\cos x+2}$}{$\ln \left\vert \frac{\tan (\frac{x}{2})}{2\tan (\frac{x}{2})+1}%
    \right\vert +C$}

  \item {$\int \frac{dx}{4\sen x+3\cos x+4}$}{$\frac{1}{3}\ln \left\vert \frac{\tan (\frac{x}{2})+1}{\tan (\frac{x}{2}%
    )+7}\right\vert +C$}

  \item {$\int \frac{\cos xdx}{\sen x+2\cos
  ...
  \item {$\int \frac{\sec x}{1+\sen x}dx$}{$-\frac{1}{2}\ln \left\vert 1-\tan ^{2}(\frac{x}{2})\right\vert +\frac{\tan (%
    \frac{x}{2})}{\left( \tan (\frac{x}{2})+1\right) ^{2}}+C$}

  \item {$\int \frac{dx}{2\sen x+\sen 2x}$}{$\frac{1}{4}\left[\ln \left\vert\tan(\frac{x}{2})\right\vert\right.$ \linebreak$\left.+\frac{\tan^{2}(\frac{x}{2})}{2}\right] +C$}

  \item {$\int \frac{dx}{2\sen x-\sen 2x}$}{$\frac{1}{4}\left( \frac{-1}{2\tan ^{2}(\frac{x}{2})}+\ln \left\vert \tan (
    \frac{x}{2})\right\vert \right) +C$}

  \item {$\int \frac{dx}{\sen x+\tan x}$}{$\frac{1}{2}\left( \ln \left\vert \tan (\frac{x}{2})\right\vert -\frac{\tan
    ^{2}(\frac{x}{2})}{2}\right) +C$}

  \item {$\int \frac{dx}{15\sen x+8\cos x+17}$}{$\linebreak\frac{-2}{9\tan (\frac{x}{2})+15}+C$}
\end{enumerate}
}
%\end{multicols}
