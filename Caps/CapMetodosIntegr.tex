...
\section[Integración trigonométrica y método de sustitución]{Integración trigonométrica\\ y método de~sustitución}
...
\subsection[Integración de funciones pares e impares en un intervalo simétrico]{Integración de funciones pares e impares\\ en un intervalo simétrico}
...
\item Si $f$ es una función par y continua en el intervalo $\left[ -a,a\right]$, entonces ${\int_{-a}^{a}f\left( x\right)dx=2\int_{0}^{a}f\left( x\right)dx}$.\label{par:1}
\item Si $f$ es una función impar y continua en el intervalo $\left[ -a,a\right]$, entonces ${\int_{-a}^{a}f\left( x\right)\,dx=0}$.\label{par:2}
\end{parrafonumerado}
\end{teorema}
...
En la figura~\ref{fig:TeoParImpar} se ilustran las partes \ref{par:1} y \ref{par:2} del teorema.

\begin{figure}[H]
\centering
\caption{Partes \ref{par:1} y \ref{par:2} del teorema}\label{fig:TeoParImpar}
...
Ahora bien, usando la tabla de integración por partes\footnote{Esta es otra manera de hacer la tabla de integración por partes que se usó atrás.}, se tiene
...
\begin{tabular}{|c|c|c|c|c|}
  ...
  \hline
  Función & $y^{3}$ & $\xrightarrow{+}$ & $e^{y}$ & $\leftarrow$ Integral\\
  \hline
...
\begin{tabular}{|c|c|c|c|c|}
  ...
  \hline
  Función & $y$ & $\xrightarrow{+}$ & $\sen y$ & $\leftarrow$ Integral \\
  \hline
  Derivada $\rightarrow$ & $1$ & $\xrightarrow{-}$ & $-\cos y$ & $\leftarrow$ Integral \\
  \hline
  Derivada $\rightarrow$ & $0$ & $\xrightarrow{+}$ & $-\sen y$ & $\leftarrow$ Integral \\
  \hline
\end{tabular}
\end{center}
...
\draw[black!60, thick] ($(C)+(-0.8,0)$) arc[start angle=180,end angle=135,radius=0.8];
\node at ($(C)+(-1,0.4)$) {$\theta$};
\node[left] at ($(A)!0.5!(B)$){$3(x+3)$};
\node[below] at ($(A)!0.5!(C)$){$\sqrt{25-9(x+3)^2}$};
\node[right, xshift=8pt] at ($(B)!0.4!(C)$) {$5$};
\end{tikzpicture}
\propia
\end{figure}
...
Para tener las expresiones del último renglón, se parte de la sustitución $x-3=2\sen(\theta)$ y se procede ya sea con identidades o usando el triángulo rectángulo (recuerde que seno es cateto opuesto sobre hipotenusa):
...
\draw[black!60, thick] ($(C)+(-0.8,0)$) arc[start angle=180,end angle=135,radius=0.8];
\node at ($(C)+(-1,0.4)$) {$\theta$};
\node[left] at ($(A)!0.5!(B)$){$x-3$};
\node[below] at ($(A)!0.5!(C)$){$\sqrt{4-(x-3)^2}$};
\node[right, xshift=8pt] at ($(B)!0.4!(C)$) {$2$};
\end{tikzpicture}
\propia
\end{figure}
...
\draw[black!60, thick] ($(C)+(-0.8,0)$) arc[start angle=180,end angle=135,radius=0.8];
\node at ($(C)+(-1,0.4)$) {$\theta$};
\node[left] at ($(A)!0.45!(B)$){$x$};
\node[below] at ($(A)!0.45!(C)$){$\sqrt{16-x^{2}}$};
\node[right, xshift=8pt] at ($(B)!0.4!(C)$) {$4$};
\end{tikzpicture}
\propia
\end{figure}
...
\int (F(a^{2}+\left( \beta (x\pm \alpha \right) )^{2})dx &= \int
  ...
  \begin{figure}[H]
    ...
    \coordinate (C) at (3,0);
    \draw[black, thick] (A) -- (B) -- (C) -- cycle;
    \draw[black!60, thick] ($(A)+(0.6,0)$) arc[start angle=0,end angle=45,radius=0.6];
    \draw[black!60, thick] (C) rectangle (2.6,0.4);
    \node at ($(A)+(0.8,0.35)$) {$\theta$};
    \node[right] at ($(B)!0.55!(C)$){$x$};
    \node[left, xshift=-4pt] at ($(A)!0.55!(B)$){$\sqrt{4+x^2}$};
    \node[below, xshift=8pt] at ($(A)!0.45!(C)$) {$2$};
    \end{tikzpicture}
    \propia
  \end{figure}
  ...
  \begin{figure}[H]
    ...
    \draw[black!60, thick] ($(C)+(-0.8,0)$) arc[start angle=180,end angle=135,radius=0.8];
    \node at ($(C)+(-1,0.4)$) {$\theta$};
    \node[left] at ($(A)!0.45!(B)$){$\sqrt{x^2-9}$};
    \node[below] at ($(A)!0.45!(C)$){$3$};
    \node[right, xshift=8pt] at ($(B)!0.4!(C)$) {$x$};
    \end{tikzpicture}
    \propia
  \end{figure}
...
\begin{figure}[H]
  ...
  \draw[black!60, thick] ($(C)+(-0.8,0)$) arc[start angle=180,end angle=135,radius=0.8];
  \node at ($(C)+(-1,0.4)$) {$\theta$};
  \node[left] at ($(A)!0.45!(B)$){$\sqrt{e^{2t}-10}$};
  \node[below] at ($(A)!0.45!(C)$){$10$};
  \node[right, xshift=8pt] at ($(B)!0.4!(C)$) {$e^t$};
  \end{tikzpicture}
  \propia
\end{figure}
...
\section[Fracciones parciales con una sustitución previa]{Fracciones parciales\\ con una sustitución previa}
...
\textls[-10]{Calcule cada una de las siguientes integrales. Use una sustitución adecuada:}
...
\begin{multicols}{2}
  ...
  \exercise{$\int \sin ^{5}y\,dy$}{$\frac{-15\cos y+10\cos^{3}y-3\cos^{5}y}{15}+C$}

  \exercise{$\int_{0}^{\frac{\pi }{2}}\cos ^{7}z\,dz$}{$\frac{16}{35}$}
  ...
  \exercise{$\int \sec ^{6}t\,dt$}{$\tan t+\frac{2\tan ^{3}t}{3}+\frac{\tan  ^{5}t}{5}+C$}

  \exercise{$\int \csc ^{4}x\,dx$}{$-\cot \left( x\right) -\frac{\cot  ^{3}\left( x\right) }{3}+C$}

  \exercise{$\int \cos ^{6}(2y)dy$}{$\frac{1}{12}\cos^{5}(2y)\sin(2y)-\frac{5}{48}\cos^{3}(2y)\sin(2y)-\frac{5}{32}\cos(2y)\sin(2y)+\frac{5}{16}y+C$}

  \exercise{$\int \sin ^{6}(z)dz$}{$-\frac{1}{6}\sin^{5}z\cos z-\frac{5}{24}\sin^{3}z\cos z-\frac{5}{16}\sin z\cos z+\frac{5}{16}z+C$}

  \exercise{$\int \tan ^{5}y\sec ^{4}y\,dy$}{$\frac{\tan ^{6}\left( y\right)}{6}+\frac{\tan ^{7}\left( y\right) }{7}+C$}

  \exercise{$\int \tan ^{7}\left( 7y\right) dy$}{$\frac{1}{7}\left(\frac{(\tan(7y))^{6}}{6}-\frac{(\tan(7y))^{4}}{4}+\frac{(\tan(7y))^{2}}{2}-\ln | \sec(7y) | \right) +C$}
  ...
  \exercise{$\int \cot ^{3}y\csc ^{4}y\,dy$}{$-\left(\frac{\cot^6(y)}{6}+\frac{\cot^4(y)}{4}\right) +C$}
  ...
  \exercise{$\int \frac{x\,dx}{\sqrt[3]{x}+\sqrt{x}}$}{$-\frac{2\sqrt{x}}{3}-\frac{2\sqrt[3]{x}}{3}-\frac{8}{9}\sqrt[6]{x}-\frac{16}{27}\ln|-3\sqrt[6]{x}+2|+C$}
...
\begin{multicols}{2}
  ...
  \exercise{${\int \frac{x+1}{2x+x^{2}}dx}$}{$\frac12\ln|2x+x^2|+C$}

  \exercise{${\int \frac{1+x}{1+x^{2}}\,dx}$}{$\frac{1}{2}\ln \left( x^{2}+1\right) +\arctan x + C$}

  \exercise{${\int \frac{x^{2}+1}{x^{3}+3x+1}\, dx}$}{$\frac{1}{3}\ln | x^{3}+3x+1 | +C$}
  ...
  \exercise{${\int xe^{2x}\, dx}$}{$\frac{1}{4}e^{2x}\left( 2x-1\right) +C$}

  \exercise{${\int x\cos x\, dx}$}{$\cos x+x\sen x+C$}

  \exercise{${\int_{0}^{\frac{\pi }{2}}x\cos (2x)\, dx}$}{$-\frac{1}{2}$}

  \exercise{${\int x^{2}e^{-x}\, dx}$}{$-e^{-x}\left( x^{2}+2x+2\right) +C$}

  \exercise{${\int x\ln x\, dx}$}{$\frac{1}{2}x^{2}\ln x-\frac{1}{4}x^{2}+C$}

  \exercise{${\int \cos x\ln (\sen x)\, dx}$}{$\left( \sen x\right) \left( \ln \left( \sen x\right)-1\right) +C$}

  \exercise{${\int \cos \left( \sqrt[3]{x}\right) \, dx}$}{$3\sqrt[3]{x}^{2}\sen
  ...
  \exercise{${\int e^{\sqrt[3]{x}}\, dx}$}{$3e^{\sqrt[3]{x}}\left( \sqrt[3]{x}^{2}-2\sqrt[3]{x}+2\right)+C $}

  \exercise{${\int \sen (\ln x)\, dx}$}{$\frac{1}{2}x\left( \sen \left( \ln x\right) -\cos\left( \ln x\right) \right) +C$}

  \exercise{${\int x\sen (\sqrt{x})\, dx}$}{ $6x\sen \sqrt{x}-2\sqrt{x}^{3}\cos \sqrt{x}-12\sen \sqrt{x}+12\sqrt{x}\cos \sqrt{x}+C$}

  \exercise{${\int \sqrt{t}\ln t\,dt}$}{$\frac{2}{3}t^{\frac{3}{2}}\ln t-\frac{4}{9}t^{\frac{3}{2}}+C$}

  \exercise{${\int x\tan ^{-1}x\, dx}$}{$\frac{1}{2}\arctan x-\frac{1}{2}x+\frac{1}{2}x^{2}\arctan x+C$}

  \exercise{${\int e^{2\theta }\sen \left( 3\theta \right) d\theta }$}{$\frac{2}{13}e^{2\theta }\sen 3\theta -\frac{3}{13}\left( \cos 3\theta \right) e^{2\theta}+C$}

  \exercise{${\int e^{-5\theta }\cos 7\theta\,d\theta }$}{$\frac{7}{74}e^{-5\theta }\sen7\theta -\frac{5}{74}\left( \cos 7\theta \right) e^{-5\theta }+C$}

  \exercise{${\int \cos \left( \ln t\right) dt}$}{$\frac{1}{2}t \left(\cos\ln t+\sen\ln t\right) +C$}
  ...
  \exercise{${\int \cot ^{5}x\sen ^{2}x\, dx}$}{$-\frac{1}{2\sen ^{2}x}\times\left( 4\sen ^{2}x\ln\sen x-\sen ^{4}x+1\right) +C$}

  \exercise{${\int_{0}^{\frac{\pi }{4}}\sec ^{6}\theta\,d\theta}$}{$\frac{28}{15}$}

  \exercise{${\int \sen \left( 4\ln x\right)\,dx}$}{ $\frac{x}{17}\left( \sen \left( 4\ln x\right) -4\cos \left( 4\ln x\right) \right)+C$}

  \exercise{${\int e^{\sqrt[4]{x}}\, dx}$}{$-4e^{\sqrt[4]{x}}\times\left( -\sqrt[4]{x}^{3}+3\sqrt[4]{x}^{2}-6\sqrt[4]{x}+6\right) +C$}

  \exercise{${\int x\ln ^{3}x\, dx}$}{$\frac{1}{8}x^{2}\left( 4\ln ^{3}x-6\ln ^{2}x+6\ln x-3\right) +C$}

  \exercise{${\int_{1}^{e}\ln ^{4}x\, dx}$}{$9e-24$ }

  \exercise{${\int \left( x^{4}+x^{2}\right) \ln x\, dx}$}{$-\frac{1}{225}
  x^{3}\left( 9x^{2}-45x^{2}\ln x-75\ln x+25\right) +C$}

  \exercise{${\int \left( x^{8}+x^{4}\right) \ln x\, dx}$}{$-\frac{1}{2025}x^{5}\left(25x^{4}-225x^{4}\ln x-405\ln x+81\right) +C$}

  \exercise{${\int \arcsen^{2}x\, dx}$}{$2\left(\arcsen x\right) \sqrt{1-x^{2}}-2x+x\arcsen ^{2}x+C$}

  \exercise{${\int x\arctan x\, dx}$}{$ \frac{1}{2}\tan^{-1} x-\frac{1}{2}x-\frac{1}{4}\pi +\frac{1}{2}x^{2}\tan^{-1} x +C$}

  \exercise{${\int x\sec^{-1} x\, dx}$}{$\frac{1}{2}\left(x^{2}\sec^{-1}x-\sqrt{x^{2}-1}\right)+C$}

  \exercise{${\int x\arcsen ^{2}x\, dx}$}{$\left( \frac{1}{8}-\frac{1}{4}\arcsen ^{2}x\right)\left( 1-2x^{2}\right) +\frac{1}{2}x\sqrt{1-x^{2}}\arcsen x+C$ }

  \exercise{${\int \tan ^{6}\theta\,d\theta }$}{$\frac{1}{5}\tan ^{5}\theta -\frac{1}{3}\tan ^{3}\theta +\tan \theta -\theta +C$}

  \exercise{${\int \tan ^{5}\theta \sec ^{3}\theta\,d\theta }$}{$\frac{1}{7}\sec ^{7}\theta -\frac{2}{5}\sec ^{5}\theta +\frac{1}{3}\sec ^{3}\theta +C$}

  \exercise{${\int \frac{dx}{x^{2}\sqrt{1-x^{2}}}}$}{ $-\frac{1}{x}\sqrt{1-x^{2}}+C$}

  \exercise{${\int \frac{dx}{x^{3}\sqrt{x^{2}-16}}}$}{$\frac{1}{128}\left( \sec^{-1}\left( \frac{x}{4}\right) +\frac{4}{x}\frac{\sqrt{x^{2}-16}}{x}\right) +C$ }

  \exercise{${\int \frac{\sqrt{9x^{2}-4}}{x}dx}$}{ $\frac{\sqrt{9x^{2}-4}}{x}- \arcsec\left(\frac{3x}{2}\right) +C$}

  \exercise{${\int x\sqrt{4-x^{2}}\, dx}$}{$-\frac{1}{3}\left( 4-x^{2}\right) ^{\frac{3}{2}}+C$}

  \exercise{${\int \frac{x^{3}}{\sqrt{x^{2}+4}}\, dx}$}{$\frac{1}{3}\sqrt{x^{2}+4}\left(x^{2}-8\right)+C$}

  \exercise{${\int \frac{dx}{x^{2}\sqrt{16x^{2}-9}}}$}{$\frac{1}{9x}\sqrt{16x^{2}-9}+C$}

  \exercise{${\int_{1}^{2}\frac{x^{2}}{\sqrt{5-x^{2}}}\, dx}$}{ $\frac{5}{2}\sen^{-1} \frac{2}{5}\sqrt{5}-\frac{5}{2}\sen^{-1} \frac{1}{5}\sqrt{5}$}

  \exercise{${\int \frac{x}{(x^{2}+4)^{\frac{5}{2}}}\, dx}$}{$-\frac{1}{3\left(x^{2}+4\right) ^{\frac{3}{2}}}+C$}

  \exercise{${\int \frac{x}{\sqrt{\left( x+1\right) ^{2}+9}}dx}$}{$\sqrt{\left( x+1\right)^{2}+9}-\ln \left\vert \frac{1}{3}\sqrt{\left( x+1\right)^{2}+9}\right.$}

  \exercise{${\int \frac{x^{2}}{\sqrt{4x-x^{2}}}\, dx}$}{$-6\sen^{-1} \left( 1-\frac{1}{2}x\right) -\frac{1}{2}x\sqrt{-x\left( x-4\right) }-3\sqrt{-x\left( x-4\right)}+C$}

  \exercise{${\int \frac{1}{\sqrt{9x^{2}+6x-8}}\, dx}$}{$\frac{1}{3}\ln\left| 3x+1+ \sqrt{9x^2+6x-8} \right| +D$}

  \exercise{${\int \frac{1}{\left( x^{2}+4x+5\right) ^{2}}dx}$}{$\frac{%
  1}{2}\left( \arctan \left( x+2\right) +\frac{x+2}{x^{2}+4x+5}\right) +C$}

  \exercise{${\int \frac{1}{(5-4x-x^{2})^{\frac{5}{2}}}\, dx}$}{$\frac{x+2}{27\sqrt{5-4x-x^2}}\left(\frac{1}{5-4x-x^2}+\frac{2}{9}\right)+C$}

  \exercise{${\int e^{t}\sqrt{9-e^{2t}}dt}$}{$\frac{e^{2t}\sqrt{9-e^{2t}}}{2}+\frac{9}{2}\arcsen\left(\frac{e^{2t}}{3}\right)+C$}

  \exercise{${\int \sqrt{e^{2t}-9}dt}$}{$3\left(\frac{\sqrt{e^{2t}-9}}{3}-\arcsec\left(\frac{e^{t}}{3}\right)\right)+C$}

  \exercise{${\int \frac{4x-1}{(x-1)(x+2)}\, dx}$}{$\ln \left| (x-1)(x+2)^3 \right| + C $}

  \exercise{${\int \frac{x^{2}+1}{x^{2}-x}\, dx}$}{$x+\ln \left|\frac{(x-1)^2}{x}\right|+C$ }

  \exercise{${\int \frac{3x^{2}-6x+2}{2x^{3}-3x^{2}+x}\, dx}$}{$\ln \left| \frac{x^2\sqrt{2x-1}}{x-1} \right| +C$}

  \exercise{${\int \frac{1}{(x-1)^{2}(x+4)}\, dx}$}{$-\frac{1}{5(x-1)}+\frac{1}{25}\ln \left\vert \frac{x+4}{x-1}\right\vert +C$}

  \exercise{${\int \frac{x^{3}}{(x+2)^{4}}\, dx}$}{$\frac{8}{3(x+2)^3}-\frac{6}{(x+2)^2}+\frac{6}{x+2}+\ln \left\vert x+2\right\vert +C$}

  \exercise{${\int \frac{1}{x^{4}-x^{2}}\, dx}$}{$\frac{1}{x}+\frac{1}{2}\ln\left\vert\frac{x-1}{x+1}\right\vert+C$}

  \exercise{${\int \frac{x}{x^{2}+x+1}\, dx}$}{$\frac{1}{2}\ln(x^2+x+1)-\frac{\sqrt{3}}{3}\arctan\left(\frac{(2x+1)\sqrt{3}}{3}\right)+D$}

  \exercise{${\int_{0}^{1}\frac{8x^{2}}{x^{4}-16}\, dx}$}{$-\ln(3)+2\arctan\left(\frac{1}{2}\right)$}

  \exercise{${\int \frac{x^{3}-2x^{2}+x+1}{x^{4}+5x^{2}+4}\, dx}$}{$-\frac{3}{2}\arctan \frac{1}{2}x+\frac{1}{2}\ln \left( x^{2}+4\right) +\arctan x+C$}

  \exercise{${\int \frac{x^{2}+1}{(x^{3}+3x)^{2}}\, dx}$}{$-\frac{1}{3\left( x^{3}+3x\right) }+C$}

  \exercise{${\int \frac{8x}{(x^{2}+4)^{3}}\, dx}$}{$-\frac{2}{\left( x^{2}+4\right) ^{2}}+C$}

  \exercise{${\int \frac{dx}{\sqrt{x}+\sqrt[3]{x}-2}}$}{$\frac{6}{5}\ln\left\vert\sqrt[6]{x}-1\right\vert+\frac{12}{5}\ln\left\vert1+\left(\sqrt[6]{x}+1\right)^{2}\right\vert-\frac{72}{5}\arctan\left(\sqrt[6]{x}+1\right)-3\sqrt[3]{x}+6\sqrt[6]{x}+2\sqrt{x}+C$}

  \exercise{$\int \frac{(4x-1)\, dx}{x(x-1)(x+2)(x+1)(x-2)}$}{$-\frac{1}{4}\ln\left\vert x\right\vert-\frac{1}{2}\ln\left\vert x-1\right\vert-\frac{3}{8}\ln\left\vert x+2\right\vert+\frac{5}{6}\ln\left\vert x+1\right\vert+\frac{7}{24}\ln\left\vert x-2\right\vert+C$}

  \exercise{${\int \frac{3x^{2}}{(x-1)(x+2)(x+3)}\, dx}$}{$\frac{1}{4}\ln\left\vert x-1\right\vert-4\ln\left\vert x+2\right\vert+\frac{27}{4}\ln\left\vert x+3\right\vert+C$}

  \exercise {$ \int \frac{dx}{\sen x-2\cos x+2}$}{$\ln\left\vert\frac{\tan(\frac{x}{2})}{2\tan(\frac{x}{2})+1}\right\vert+C$}

  \columnbreak

  \exercise {$ \int \frac{dx}{4\sen x+3\cos x+4}$}{$\frac{1}{3}\ln\left\vert\frac{\tan(\frac{x}{2})+1}{\tan(\frac{x}{2})+7}\right\vert+C$}

  \exercise {$ \int \frac{\cos x\,dx}{\sen x+2\cos x-2}$}{$\ln\left\vert\frac{\tan(\frac{x}{2})}{\left(2\tan(\frac{x}{2})-1\right)^{\frac{3}{5}}\left(1+\tan^{2}(\frac{x}{2})\right)^{\frac{1}{5}}}\right\vert+\frac{2x}{5}+C$}

  \exercise{$\int \frac{\sec x}{1+\sen x}dx$}{$-\frac{1}{2}\ln\left\vert1-\tan^{2}(\frac{x}{2})\right\vert+\frac{\tan(\frac{x}{2})}{\left(\tan(\frac{x}{2})+1\right)^{2}}+C$}
  ...
  \exercise{$\int \frac{dx}{\sen x+\tan x}$}{$\frac{1}{2}\left(\ln\left\vert\tan(\frac{x}{2})\right\vert-\frac{\tan^{2}(\frac{x}{2})}{2}\right)+C$}
