...
\begin{ejemplo}
  \textnormal{Si se quiere calcular la integral {\footnotesize $\displaystyle{\int e^{x^2}xdx}$}, donde el $x$ que multiplica a la exponencial es la derivada (salvo el $2$) de $x^2$, entonces podemos hacer la sustitución}.
\end{ejemplo}
\[
...
\begin{ejemplo}
\textnormal{Suponga que $f$ es una función par y que {\small \textit{$\displaystyle{\int_{0}^{32}f\left( x\right)
dx=20.}$}} Evalúe}
\[
...
\emph{Solución.} Si en {\footnotesize $\displaystyle{\int_{-2}^{2}x^{4}f\left( x^{5}\right) dx}$} hacemos la sustitución $\ u=x^{5}\rightarrow du=5x^{4}dx\rightarrow x^{4}dx=\frac{1}{5}du$ y cambiamos límites de integración. Si $x=-2$, entonces {\footnotesize $u=\left( -2\right)^{5}=-32$} y, si $x=2$, entonces {\footnotesize $u=2^{5}=32$.}
\[
...
\begin{ejemplo}
\textnormal{La integral $\displaystyle{\int e^{\sqrt{x}}dx}$, inicialmente, no tiene la forma del caso anterior, pero puede reescribirse:}
\end{ejemplo}
