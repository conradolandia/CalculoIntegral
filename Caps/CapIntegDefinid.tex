...
\item Para este caso se usan las propiedades asociativa y conmutativa de números reales, y se tiene
  \begin{align*}
    ...
    &=\overset{n}{\underset{k=1}{\sum }}a_{k}+\overset{n}{\underset{k=1}{\sum }}b_{k}
  \end{align*}
...
\item Al manipular las expresiones se tiene
  \begin{align*}
    ...
    &=\overset{n}{\underset{k=1}{\sum }}a_{k}+\overset{n}{\underset{k=1}{\sum }}\left( -1\right) b_{k} \quad \text{\small (Propiedad homogénea)}\\
    &=\overset{n}{\underset{k=1}{\sum }}a_{k}+\left(-1\right) \overset{n}{\underset{k=1}{\sum }}b_{k}\\
    &=\overset{n}{\underset{k=1}{\sum }}a_{k}-\overset{n}{\underset{k=1}{\sum }}b_{k}
  \end{align*}
...
\textit{Demostración}. Usemos inducción\footnote{Puede consultarse Stewart~\textcite{Stewart_2012}, páginas 820-827.} sobre $m.$
...
\item Supongamos ahora que el resultado es válido para todo número natural menor o igual a $m$ y
  ...
  \end{equation}
  y por otro lado, usando el teorema del binomio\footnote{Para recordar sobre inducción matemática, puede consultarse Stewart~\textcite{Stewart_2012}, páginas 814-819.},
...
\item $\sum\limits_{k=1}^{n}\cos kx=\cfrac{\sen\left(( n+\frac{1}{2})x\right)-\sen\frac{x}{2} }{2\sen\frac{x}{2} }$
...
Con ello, el $k$-ésimo rectángulo tiene área
\[
...
\begin{align*}
a\left( \mathcal{R}\right)& =\underset{n\rightarrow \infty }{\lim }\left[ \frac{16}{n^{4}}\left(\frac{n(n+1)}{2}\right)^2 +4\right]\\ \displaybreak
...
\]
encuentre los subintervalos y \textit{$\left\Vert P\right\Vert $.}
...
\Delta x_{2}&=-1+\frac{7}{5}=\frac{2}{5}\\
...
\item ${\int_{a}^{b}f(x)dx}=\underset{n\rightarrow \infty }{\lim }\sum_{k=1}^{n}f(x_{k})\Delta x,$ con $P=\{x_{0},x_{1},x_{2},\cdots ,x_{n}\}$ una partición regular del intervalo $[a,b]$, $\Delta x=\frac{b-a}{n}$\; y\; $x_{k}=a+k\Delta x=a+k\frac{b-a}{n},$ $1\leq k\leq n$.
\end{parrafonumerado}
\end{teorema}

\begin{ejemplo} Una función que no es integrable. Tomemos el siguiente caso. Supongamos que $0\leq x\leq 3$ y que
\[
f(x)=
\begin{cases}
...
\item Propiedad homogénea: \[\int_{a}^{b}\alpha f(x)dx=\alpha \int_{a}^{b}f(x)dx\]
\item Propiedad de linealidad: \[\int_{a}^{b}\left( \alpha f(x)+\beta g(x)\right) dx=\alpha \int_{a}^{b}f(x)dx+\beta \int_{a}^{b}g(x)dx\]
\item Propiedad de aditividad en el intervalo: \[\int_{a}^{b}f(x)dx=\int_{a}^{c}f(x)dx+\int_{c}^{b}f(x)dx\] para $c$ entre $a$ y $b$\footnote{La condición de que $c$ está entre $a$ y $b$ no es necesaria. Acá se incluye solo por ser el caso que se toma en la demostración de más adelante}
\item Positividad: $f\geq 0$ en $[a,b]$, entonces $\int_{a}^{b}f(x)dx\geq 0$.
\item Monotonía: sean $f$ y $g$ tales que $f\left( x\right)\leq g\left( x\right) $ para todo $x\in \left[ a,b\right],$ entonces \textit{$\int_{a}^{b}f(x)dx\leq \int_{a}^{b}g(x)dx$.}
...
\begin{teorema}
\begin{parrafonumerado}
\item Si $f$unción continua en $[a,b]$s $f$
uniformemente continua en $[a,b]$. Es decir, dado un $\varepsilon >0$,
existe un $\delta >0$ tal que para cada par de puntos $x,\ y$ en $[a,b]$, si $\left\vert x-y\right\vert <\delta$, entonces $\left\vert f\left( x\right)-f\left( y\right) \right\vert <\varepsilon$.
...
\textls[-15]{Ahora bien, por inducción matemática, tenemos entonces que para $\delta _{n}=\frac{b-a}{2^{n}}$ existen $a_{n},b_{n}$ en $[a,b]$ con
...
\textls[-15]{Además, como $f$ es continua en $[a,b]$, entonces $f$ es continua en $\alpha $; luego, para $\varepsilon _{0}$ existe $\delta ,$ tal que, si $x\in \left( \alpha
...
\begin{align*}
\left\vert f\left( b_{n}\right) -f\left( a_{n}\right) \right\vert
&=\left\vert f\left( b_{n}\right) -\alpha +\alpha -f\left( a_{n}\right) \right\vert\\
&\leq \left\vert f\left( b_{n}\right) -\alpha \right\vert
...
Supongamos ahora que $\delta =\text{máx}\left\{ \delta _{k}:1\leq k\leq n\right\}$.
...
\item Tomemos ahora algunas funciones trigonométricas. Iniciemos con $\cos x$,
\begin{equation*}
\int_{0}^{b}\cos x\,dx=\underset{n\rightarrow \infty }{\lim }\sum\limits_{k=1}^{n}\cos \left( k\frac{b}{n}\right) \frac{b}{n}
=\underset{n\rightarrow \infty }{\lim }\frac{b}{n}\sum\limits_{k=1}^{n}
\cos \left( k\frac{b}{n}\right)
\end{equation*}
aplicando el teorema \ref{SumSenoCoseno}, $a$ con $x=\frac{b}{n}$, resulta
\begin{equation*}
...
\item { Para finalizar, tomemos una función exponencial con base $b$, es decir, $f(x)=b^x$, y calculemos la \textit{$\int_\alpha^\beta b^x$:}}
\begin{align*}
...
\begin{multicols}{2}
\begin{parrafonumerado}
\item ${\int_{1}^{4}3x^{3}-2x+1 dx}$
\item ${\int_{\frac{\pi }{6}}^{\frac{\pi }{2}}\cos xdx}$
\item ${\int_{\frac{\pi }{3}}^{\frac{\pi }{2}}\sin xdx}$
\item ${\int_{-1}^{2}2^{2x}dx}$
\end{parrafonumerado}
\end{multicols}
...
\emph{Solución.} Con la figura señalando la región entre $-2$ y $2$:
...
\begin{ejemplo}
Determine el valor de $\int_{1}^{\frac{9}{2}}\big[\hspace{-0.06cm}| x|\hspace{-0.07cm}\big] dx.$
\end{ejemplo}

Recordemos que $\big[\hspace{-0.05cm}| x|\hspace{-0.07cm}\big]$ es la parte entera de $x$, es decir, el entero más cercano a $x$ por su izquierda. Por ejemplo, {\small $\big[\hspace{-0.05cm}| 2.3|\hspace{-0.07cm}\big]=2$}. La figura de $f(x)=\big[\hspace{-0.05cm}| x|\hspace{-0.07cm}\big]$ se muestra a continuación:
...
\emph{Solución.} La función inversa de $y=\sqrt[3]{x}$ es $x=y^{3}$. Según la
...
igual a {\footnotesize $\int_{0}^{2}y^{3}dy$}. Entonces,
...
\textnormal{Si $f$ es positiva, creciente e integrable en el intervalo $\left[ 0,x_0%
\right]$, entonces}
\[
...
% \subexercise{$\int_{0}^{1}\left(x^{2}-x\right) dx$}{$-\dfrac{1}{6}$}

% \subexercise{$\int_{-1}^{1}x^{3}dx$}{$0$}

% \subexercise{$\int_{-2}^{2}\left( x^{3}+2\right) dx$}{$8$}

% \subexercise{$\int_{-1}^{2}\left( 2x^{3}-1\right) dx$}{$\dfrac{9}{2}$}

% \subexercise{$\int_{0}^{2}\left(2^{x}\right) dx$}{$\dfrac{3}{\ln 2}$}

% \subexercise{$\int_{0}^{3}2\left( 3^{x}\right) dx$}{$\dfrac{52}{\ln 3}$}
% %\end{multicols}
...
a. $\int_{0}^{2}\left( x^{2}-2\right) dx$
% $-\dfrac{4}{3}$

b. $\int_{0}^{1}\left(x^{2}-x\right) dx$
% $-\dfrac{1}{6}$
...
\begin{minipage}[t]{\linewidth}
d. $\int_{-2}^{2}\left( x^{3}+2\right) dx$
% $8$

e. $\int_{-1}^{2}\left( 2x^{3}-1\right) dx$
% $\dfrac{9}{2}$
\end{minipage}
...
\begin{multicols}{2}
a. $\int_{0}^{2}\left( 4x^{3}-8x^{2}+7x+8\right) dx$

b. $\int_{-2}^{1}\left( 5x^{3}-7x^{2}+4x-3\right) dx$

c. $\int_{-2}^{2}\left( x^{4}+2x^{3}-x+1\right) dx$

d. $\int_{-1}^{2}\left( 2x^{5}-10x^{3}\right) dx$

e. $\int_{0}^{3}\left( e^{x}+2^{x}+3\right) dx$

f. $\int_{0}^{3}\left( 4^{x}+3^{x}+3x\right) dx$

g. $\int_{0}^{3}\left(5^{x}+4^{x}+4x^{2}\right) dx$

h. $\int_{-1}^{2}\left\vert x\right\vert dx$

i. $\int_{-1}^{4}\left\vert x^{2}-2x-3\right\vert dx$

j. $\int_{-4}^{4}\left\vert x^{2}-9\right\vert dx$

k. $\int_{-1}^{2}\left[ \left\vert x\right\vert \right] dx$

l. $\int_{1}^{4}\left[ \left\vert 2x\right\vert \right] dx$

m. $\int_{-1}^{4}\left[ \left\vert 3x\right\vert \right] dx$

n. $\int_{0}^{4}\left[ \left\vert x^{2}\right\vert \right] dx$

o. $\int_{0}^{2}\left[ \left\vert 2x^{2}\right\vert \right] dx$

p. $\int_{0}^{2}\left[ \left\vert x^{3}\right\vert \right] dx$

q. $\int_{-2}^{2}f(x)\,dx$, si $f(x)=
\begin{cases}
x^{2}-4, & \text{si }x<0 \\
4-x^{2}, & \text{si }x\geq 0
\end{cases}$

r. $\int_{-1}^{3}f(x)\,dx$, si $f(x)=
\begin{cases}
x^{3}-5x^{2}+x+2, & \text{si }x<1 \\ % <-- ¡AQUÍ ESTÁ EL CAMBIO!
4x^{3}-x^{2}+2, & \text{si }x\geq 1
\end{cases}$

s. $\int_{-2}^{3}f(x)\,dx$, si $f(x)=
\begin{cases}
...
% %\begin{multicols}{2}
% \subexercise{$\int_{-2}^{2}\sqrt{4-x^{2}}dx$}{$2\pi$}

% \subexercise{$\int_{0}^{10}\sqrt{100-x^{2}}dx$}{$25\pi$}

% \subexercise{$\int_{-2}^{8}\left( 4+\sqrt{25-\left( x-3\right) ^{2}}\right) dx$}{$\frac{25\pi}{2}+40$}

% \subexercise{$\int_{-1}^{6}\left( 1+\sqrt{49-\left( x+1\right) ^{2}}\right) dx$}{$\frac{49\pi}{4}+7$}

% \subexercise{$\int_{0}^{4}\big[\hspace{-0.05cm}| x|\hspace{-0.07cm}\big] dx$}{$6$}

% \subexercise{$\int_{0}^{3}\big[\hspace{-0.05cm}| 2x|\hspace{-0.07cm}\big] dx$}{$7,5$}

% \subexercise{$\int_{0}^{2}\big[\hspace{-0.05cm}| x^2|\hspace{-0.07cm}\big] dx$}{$1,8537$}

% \subexercise{$\int_{0}^{9} \big[\hspace{-0.05cm}| \sqrt{x}|\hspace{-0.07cm}\big] dx$}{$13$}
%\end{multicols}
...
\begin{ejer}{Use nociones geométricas para encontrar el valor de cada una de las siguientes integrales:}
  ...
  \begin{multicols}{2}
    ...
    b. $\int_{0}^{10}\sqrt{100-x^{2}}dx$
    % $25\pi$

    c. $\int_{-2}^{8}\left( 4+\sqrt{25-\left( x-3\right) ^{2}}\right) dx$
    % $\dfrac{25\pi}{2}+40$

    d. $\int_{-1}^{6}\left( 1+\sqrt{49-\left( x+1\right) ^{2}}\right) dx$
    % $\dfrac{49\pi}{4}+7$

    e. $\int_{0}^{4}\big[\hspace{-0.05cm}| x|\hspace{-0.07cm}\big] dx$
    % $6$

    f. $\int_{0}^{3}\big[\hspace{-0.05cm}| 2x|\hspace{-0.07cm}\big] dx$
    % $7{,}5$

    g. $\int_{0}^{2}\big[\hspace{-0.05cm}| x^2|\hspace{-0.07cm}\big] dx$
    % $1{,}8537$

    h. $\int_{0}^{9} \big[\hspace{-0.05cm}| \sqrt{x}|\hspace{-0.07cm}\big] dx$
    % $13$
  \end{multicols}
\end{ejer}
% \exercise{Calcule las integrales usando propiedades de áreas y de funciones inversas:}
% %\begin{multicols}{3}
% \subexercise{$\int_{0}^{9}\sqrt{x}dx$}{$18$}

% \subexercise{$\int_{0}^{16}\sqrt[4]{x}dx$}{$\frac{128}{5}$}

% \subexercise{$\int_{0}^{4}\sqrt[5]{8x}dx$}{$\frac{20}{3}$}

% \subexercise{$\int_{0}^{7}\log _{2}\left( x+1\right) dx$}{$ 24-\frac{7}{\ln 2}$}

% \subexercise{$\int_{0}^{e-1}\ln \left( x+1\right) dx$}{$ 1$}

% \subexercise{$\int_{1}^{25}\log _{5}xdx$}{$50-\frac{24}{\ln 5}$}

% \subexercise{$\int_{0}^{\frac{1}{2}}\arcsin \left( 2x\right) dx$}{$\frac{1}{4}\pi -\frac{1}{2}$}

% \subexercise{$\int_{0}^{3}\arcsin \left( \frac{x}{3}\right) dx$}{$\frac{3}{2}\pi-3$}

% \subexercise{$\int_{0}^{1}\arccos \left( 1-x\right) dx$}{$1$}
% %\end{multicols}
...
\begin{ejer}{Calcule las integrales usando propiedades de áreas y de funciones inversas:}
  ...
  \begin{multicols}{3}
    ...
    b. $\int_{0}^{16}\sqrt[4]{x}\,dx$
    % $\dfrac{128}{5}$

    c. $\int_{0}^{4}\sqrt[5]{8x}\,dx$
    % $\dfrac{20}{3}$

    d. $\int_{0}^{7}\log_{2}(x+1)\,dx$
    % $24-\dfrac{7}{\ln 2}$

    e. $\int_{0}^{e-1}\ln(x+1)\,dx$
    % $1$

    f. $\int_{1}^{25}\log_{5}x\,dx$
    % $50-\dfrac{24}{\ln 5}$

    g. $\int_{0}^{\frac{1}{2}}\arcsin(2x)\,dx$
    % $\dfrac{1}{4}\pi - \dfrac{1}{2}$

    h. $\int_{0}^{3}\arcsin\left(\dfrac{x}{3}\right)\,dx$
    % $\dfrac{3}{2}\pi - 3$

    i. $\int_{0}^{1}\arccos(1 - x)\,dx$
    % $1$
  \end{multicols}
\end{ejer}
