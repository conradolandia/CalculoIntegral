...
%\includegraphics[scale=0.35]{GrafAreaEntreCurvFx.pdf}
...
%\includegraphics[scale=0.25]{GrafAreaEntreCurvFy.pdf}
%\textbf{Fuente:} elaboración propia
%\end{center}
...
\begin{ejemplo}
\textnormal{Cuando una partícula se localiza a una distancia de \textit{$x$} metros desde el origen, una fuerza de \textit{$F\left( x\right) =\cos \left( \frac{\pi x}{3}\right) $} newtons actúa sobre ella. \textquestiondown Cuánto trabajo se realiza al mover la partícula desde \textit{$x=0$} hasta \textit{$x=2?$}}
\end{ejemplo}
...
\begin{ejemplo} \textnormal{\textbf{Presión y fuerza:} una placa rectangular plana de $5 \times 6$ pies se sumerge horizontalmente en agua a una profundidad de $10$ pies. Determine la presión y la fuerza ejercidas sobre la placa por el agua arriba de esta.}
\end{ejemplo}
...
\textit{Solución}. Recuerde que el peso específico del agua es $62,4$ lb/pie
...
\begin{defi}
\textnormal{Sea \textit{$\rho $} el peso específico de un fluido y sea \textit{$w(x)$} una función continua sobre \textit{$[a,b]$} que describe el ancho de una placa plana sumergida verticalmente a una profundidad \textit{$x$}. La fuerza \textit{$F$} ejercida por el fluido sobre un lado de la placa sumergida es}
\[
